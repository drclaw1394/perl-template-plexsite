...
add resources required, import modules and define local helper code and
variables.  L<Template::Plex> is used in the back end to power this module, so
all the features such as init blocks, inheritance, sub templates, etc are used
and extended.

=head1 SO WHAT DOES IT DO?

Specifically, L<Template::Plexsite> provides the boot strapping templates need
to reference other templates by their original name and locaiton. The final
...
syntax highlighting for known file types for exampl.e
